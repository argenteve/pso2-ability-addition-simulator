...
Ext.define('PSO2.CostPanel', {
	...
	initItems: function() {
		...
		costFieldItems.push({
			...
			handler: function() {
				...
				Ext.Msg.alert('情報', 'クッキーへ値を保存しました。');
			}
		});
	...
	addCost: function(comp) {
		var me = this, rp = comp.getResultPanel(),
			...
			items = [{
				name: rp.id + me.constResultPanel + '-cur-0',
				fieldLabel: '素体'
			}], newPanel, i;

		for (i = 1; i <= me.maxMaterial; i++) {
			items.push({
				name: rp.id + me.constResultPanel + '-cur-' + i,
				fieldLabel: '素材' + i
			});
		}
					...
					items: [{
						...
						tpl: new Ext.XTemplate([
							'<div class="x-cost-frame">',
								...
								'<div class="x-cost-label">期待値:</div>',
								'<div class="x-cost-value">{text}</div>',
							'</div>',
							'<div style="clear:both"></div>'
						]),
	...
	registReferenceCost: function(expPanel, currencyField) {
		...
		if (ret) {
			Ext.Msg.alert('警告', '循環参照が発生した為、処理を継続できませんでした。');
